...
// content tabs init
function initTabs() {
	jQuery('ul.tabset-01').contentTabs({
		parentBlock: '.open-close-02',
		tabLinks: 'a'
	});
	jQuery('ul.tabset-02').contentTabs({
		parentBlock: '.open-close-03',
		tabLinks: 'a',
		effect: 'fade',
		extFlag: true
	});
...
function initTextChange () {
	jQuery('.tab-page').each(function() {
		var holder = jQuery(this);
		if (!holder.length) {
			return;
		}
		var openHold = holder.find('.open-close-03');
	});
}
...
function initDropDown() {
	...
	var drop = nav.find('ul');
	var winWidth;
	var resizeFlag = false;
	var resizeFlag2 = false;
	calcWinWidth();
	win.on('resize', calcWinWidth);
	function calcWinWidth () {
		winWidth = win.outerWidth();
		if (winWidth < 768 && !resizeFlag) {
			items.each(function () {
				var item = jQuery(this),
					dropWrap = item.find('.drop-slide-wrapper'),
					drop = item.find('ul');
				item.removeAttr('class');
				drop.removeAttr('style').appendTo(item);
				item.unbind(window.TouchNav && TouchNav.isActiveOn(this) ? 'itemhover' : 'mouseenter').unbind(window.TouchNav && TouchNav.isActiveOn(this) ? 'itemleave' : 'mouseleave');
				if (dropWrap.length) {
					dropWrap.remove();
				}
			});
...
function initOpenClose() {
	jQuery('div.open-close-02').openClose({
	...
	jQuery('div.open-close-03').openClose({
		hideOnClickOutside: true,
		activeClass: 'active',
		opener: '.opener-04',
		slider: '.slide-03',
...
function initAccordion() {
	jQuery('ul.accordion-01').slideAccordion({
		opener: 'a.opener-01',
		slider: 'div.slide-01',
		animSpeed: 300
	});
	jQuery('div.four-columns').slideAccordion({
		opener: 'h3.opener-03',
		slider: 'div.slide-02',
		animSpeed: 300
	});
...
;(function($){
	$.fn.contentTabs = function(o){
		...
		return this.each(function(){
			...
			var extClass = 'non-block';
			var textLink = jQuery(options.parentBlock).children('a');
			...
			tabLinks.each(function(){
				...
				if(classOwner.hasClass(options.activeClass) || (options.checkHash && location.hash === href)) {
					...
					if (options.extFlag == true && tabLinks.eq(0).hasClass(options.activeClass)) {
						mainBlock.addClass(extClass);
					}
			...
			function switchTab(oldLink, newLink) {
				...
				textLink.text(newLink.text());
				var winWidth = win.outerWidth();
				if (winWidth < 768) {
					jQuery(options.parentBlock).data('OpenClose').hideSlide();
				}
				currentTab = newTab;
				...
				if(options.extFlag == true){
					if (tabLinks.eq(0).hasClass(options.activeClass)) {
						mainBlock.addClass(extClass);
					} else {
						mainBlock.removeClass(extClass);
					}
				}					
